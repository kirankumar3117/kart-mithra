...
import { ListChecks, Bot, PackageCheck, User, Globe, Linkedin, Github, Bike, Truck, ArrowRight, ArrowDown, Sparkles, X, Send, Smartphone, Store, CheckCircle, Clock } from "lucide-react";
...
export default function VisionPage() {
  ...
  return (
    ...
    <section className="py-24 px-4 bg-surface-card/20 border-b border-surface-border overflow-hidden relative">
      ...
      <div className="max-w-5xl mx-auto relative z-10 hidden sm:block">
        <motion.div {...animations.fadeInUp} className="text-center mb-16">
          <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4 tracking-tight">The Experience</h2>
          <p className="text-muted max-w-2xl mx-auto text-lg">Customer orders from mobile. Shopkeeper accepts. You don't wait.</p>
        </motion.div>

        <div className="relative flex items-center justify-between w-full max-w-4xl mx-auto h-[400px]">
          ...
          <motion.div 
            ...
            className="w-72 h-[380px] bg-surface rounded-[2rem] border-8 border-surface-card p-4 relative shadow-2xl flex flex-col pt-8"
          >
            {/* Dynamic island */}
            ...
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="text-sm font-semibold text-white">{t.vision.animCustomerApp}</span>
            </div>

            <div className="flex-1 space-y-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex gap-3 items-center bg-surface-card/50 p-2 rounded-xl">
            ...
            <AnimatePresence mode="popLayout">
              {animStage >= 1 && (
                ...
                >
                  <span className="text-brand font-medium tracking-wide">{t.vision.animOrderPlaced}</span>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
            ...
            <AnimatePresence>
              {animStage >= 4 && (
                <motion.div 
                  ...
                  className="absolute bottom-20 bg-green-500/10 border border-green-500/30 text-green-400 px-6 py-3 rounded-full font-bold shadow-[0_0_30px_rgba(34,197,94,0.3)] flex items-center gap-2 backdrop-blur-md"
                >
          ...
          <motion.div 
            ...
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8 }}
            className="w-80 h-[380px] bg-surface-card rounded-2xl border border-surface-border relative shadow-2xl flex flex-col overflow-hidden"
          >
            <div className="h-14 border-b border-surface-border flex items-center px-4 gap-3 bg-surface/50">
              <Store className="w-5 h-5 text-accent" />
              <span className="text-sm font-semibold text-white">{t.vision.animMerchantApp}</span>
            </div>
              ...
              <AnimatePresence mode="popLayout">
                {animStage >= 2 && animStage < 4 && (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-white font-medium">{t.vision.animNewOrder}</span>
                    </div>
                    {animStage === 3 && (
                      <motion.div 
                        initial={{ opacity: 0 }}
                ...
                {animStage >= 4 && (
                  ...
                  >
                    ...
                    <span className="text-green-400 font-bold text-lg">{t.vision.animReady}</span>
                  </motion.div>
                )}
              </AnimatePresence>
